 ...
 *
 * Modifiqued by Guilherme Biazi Sabin with 2016/08/02
 */
...
/*
// Command:                  RF_POWER_UP
// Description:              Power-up the device with the specified function. 
//                           Power-up is complete when the CTS bit is set. 
//                           This command may take longer to set the CTS bit than other commands.
// Configuration:            0x02 - CMD
//                           0x01 - Copy selected functional image from OTP and boot device.
//                                  Boot main application image
//                           0x00 - TCXO not used, XTAL used.
                             0xC9,0xC3,0x80 -  Frequency of TCXO or external crystal oscillator in Hz.
*/
...
// SDO
*/
#define RF_GPIO_PIN_CFG 0x13, 0x14, 0x02, 0x21, 0x20, 0x27, 0x0B, 0x00
...
// Default values:           0x40, 
// Descriptions:
//   GLOBAL_XO_TUNE - Crystal oscillator frequency tuning value. 
//   0x00 is maximum frequency value and 0x7F is lowest frequency value. 
//   Each LSB code corresponds to a 70 fF capacitance change. 
//   The total adjustment range assuming a 30 MHz XTAL is +/-100 ppm.
*/
#define RF_GLOBAL_XO_TUNE_1 0x11, 0x00, 0x01, 0x00, 0x62
...
// Start ID:             03
// Default values:       0x20
// Configured:           FIFO MODE  - TX/RX FIFO are sharing with 128-byte size buffer.
                         PROTOCOL   - Packet format is generic, no dynamic reprogramming of packet handler properties
                         POWER_MODE - High performance mode for RX and TX. RX current = 13 mA.
// Descriptions:
//   GLOBAL_CONFIG - Various settings that affect entire chip. 
//   If PROTOCOL is specified, the chip is placed into protocol aware state.
*/
...
// Descriptions:
//   INT_CTL_ENABLE - Enables top-level interrupt sources to generate HW interrupts at the NIRQ pin. 
//   The three interrupt groups are Chip, Modem and Packet Handler. 
//   Each of them contains multiple possible interrupt sources that must be individually enabled via the INT_CTL_PH_ENABLE, 
//   INT_CTL_MODEM_ENABLE, INT_CTL_CHIP_ENABLE properties. 
//   Note that this property only provides for global enabling/disabling of the HW interrupt indication on the NIRQ output pin.
//   An internal interrupt event may still be generated even if the indication on the NIRQ output pin is disabled.
//   The interrupt event may always be monitored by polling a GPIO pin, or via GET_INT_STATUS, GET_CHIP_STATUS, GET_PH_STATUS, 
//   or GET_MODEM_STATUS commands.
//   INT_CTL_PH_ENABLE - Enable individual interrupt sources within the Packet Handler Interrupt Group in order to generate a
//   HW interrupt at the NIRQ output pin. In order to fully enable a HW interrupt, it is necessary to enable both the individual 
//   interrupt source (within this property) as well as the corresponding interrupt group (e.g., set INT_CTL_ENABLE:PH_INT_STATUS_EN). 
//   Note that even if an interrupt source is not enabled to generate a HW NIRQ interrupt, the given interrupt event still may 
//   occur inside the chip and may be monitored by polling a GPIO pin, or via the GET_INT_STATUS or GET_PH_STATUS commands.
*/
...
// Descriptions:
//   PREAMBLE_TX_LENGTH - This property is used to configure the length of the Preamble field in TX mode for both Standard 
//    and Non-Standard Preambles. This property is not used by the chip in RX mode except when receiving a Non-Standard Preamble.
//    In such a case, this property must be configured with the expected Preamble length (to provide an upper timeout limit on 
//    the Sync Word search algorithm). This property is only applicable in TX Packet Handler FIFO mode; if TX Direct Synchronous 
//    or TX Direct Asynchronous mode is selected (see MODEM_MOD_TYPE property), the entire transmission is obtained from a 
//    real-time TXDATA input stream on a GPIO pin and no automatic field construction is possible. The units of this property are 
//    in either nibbles or bytes, depending upon the value of PREAMBLE_CONFIG:LENGTH_CONFIG. Setting PREAMBLE_TX_LENGTH = 0x00 
//    is a valid value, and will result in skipping transmission of the Preamble field; in such a case, the Sync Word will be the 
//    first transmitted field.
//    PREAMBLE_CONFIG_STD_1 - The fields in this property are applicable only in RX mode, and apply primarily to reception of 
//    Standard Preamble patterns (e.g., 1010 or 0101 patterns). However, see note on required configuration of RX_THRESH during
//    reception of Non-Standard Preamble patterns.
//    PREAMBLE_CONFIG_NSTD - The fields in this property apply only to transmission and reception of packets with Non-Standard 
//    Preamble patterns (e.g., other than 1010 or 0101 patterns).
//    PREAMBLE_CONFIG_STD_2 - During initial reception and acquisition of a packet, it is often useful to define a timeout period 
//    to limit the amount of time the chip remains on-channel awaiting the incoming packet. This property allows configuration of 
//    this preamble search timeout period. If a valid preamble is not found within the specified timeout period, an 
//    INVALID_PREAMBLE event is generated and may be used to generate an interrupt and/or exit automatically from RX mode. 
//    The fields in this property are applicable only in RX mode, and only apply to reception of Standard Preamble patterns 
//    (e.g., 1010 or 0101 patterns). Detection of INVALID_PREAMBLE is not possible when configured to receive a Non-Standard 
//    Preamble pattern. Two different timeout configuration fields are provided to allow definition of timeout periods with 
//    significantly different lengths and bit resolution.
//    PREAMBLE_CONFIG - Miscellaneous preamble configuration bits.
//    PREAMBLE_PATTERN_31_24 - The PREAMBLE_PATTERN_XX properties define the pattern to be transmitted in TX mode and the pattern 
//    expected to be received in RX mode, but only when PREAMBLE_CONFIG:STANDARD_PREAM has been set to 0x0 (i.e., use Non-Standard 
//    Preamble). The PREAMBLE_PATTERN_XX properties are used to define the actual bit values describing the Non-Standard Preamble 
//    pattern; PREAMBLE_CONFIG_NSTD:PATTERN_LENGTH is used to define how many bits of the PREAMBLE_PATTERN_XX values are used. 
//    In TX mode, if the value of PREAMBLE_TX_LENGTH is larger than PREAMBLE_CONFIG_NSTD:PATTERN_LENGTH, the PREAMBLE_PATTERN 
//    will be transmitted repeatedly until the full number of bits has been sent. In RX mode, the full number of bits defined 
//    by PREAMBLE_CONFIG_NSTD:PATTERN_LENGTH must be received once before the PREAMBLE_VALID signal/interrupt is generated. 
//    If Manchester encoding/decoding is enabled, the values of the PREAMBLE_PATTERN_XX properties are expressed in chips 
//    (i.e., after Manchester encoding in TX mode, or before Manchester decoding in RX mode). The bits of the PREAMBLE_PATTERN_XX 
//    are always sent bits 0-31 time-wise (e.g., PREAMBLE_PATTERN_7_0 bit 0 is sent first).
//    PREAMBLE_PATTERN_23_16 - Bits 23-16 of the Non-Standard Preamble pattern to be transmitted or expected to be received. 
//    Please refer to the text description for PREAMBLE_PATTERN_31_24 for more details regarding the Non-Standard Preamble functionality.
//    PREAMBLE_PATTERN_15_8 - Bits 15-8 of the Non-Standard Preamble pattern to be transmitted or expected to be received. 
//    Please refer to the text description for PREAMBLE_PATTERN_31_24 for more details regarding the Non-Standard Preamble functionality.
//    PREAMBLE_PATTERN_7_0 - Bits 7-0 of the Non-Standard Preamble pattern to be transmitted or expected to be received. 
//    Please refer to the text description for PREAMBLE_PATTERN_31_24 for more details regarding the Non-Standard Preamble functionality.
// Configuration: 1000 - 0x08 - Length of preamble
                  1001 - 0x14 - Number of preamble bits that must be valid to detect a valid preamble 
                  1002 - 0x00 - 
                  1003 - 0x0F - Number of nibbles to search for before determining that a preamble does not exist.
                  1004 - 0x31 - calculated from the calculator, Preamble tx_length register is in bytes,
                  Preamble tx_length register is in bytes.
                  When Manchester is enabled, if preamble pattern is 0101, the post-Manchester transmitted bits will be
                  Preamble is not manchester encoded.
                  Use standard preamble of 1010.
                  1005 - 0x00 -
                  1006 - 0x00 -
                  1007 - 0x00 -
                  1008 - 0x00 -
...
// Descriptions:
//   SYNC_CONFIG - Configuration of miscellaneous Sync Word bits. NOTE: each byte of the Sync Word is transmitted/received in 
//   little-endian fashion (i.e., least significant bit first). Byte(s) of the Sync Word are transmitted/received in descending order 
//   (i.e., Byte 3 first, followed by Byte 2, etc.)
//   SYNC_BITS_31_24 - Define the value of Byte 3 of the Sync Word for both TX and RX operation. NOTE: Although the Sync Word byte(s)
//   are transmitted/received in descending order (i.e., Byte 3 first, followed by Byte 2, etc.), each byte is transmitted/received 
//   in little-endian fashion (i.e., least significant bit first).
...
// Configurations: 1100 - 0x01 - Sync word is transmitted as defined by LENGTH field.
                                 default:0x0 Number of sync bit errors that are allowed in the sync field during receive sync detection.
                                 Sync word is not 4FSK modulated.
                                 Sync word is not manchester encoded.
                                 Sync word is 16 bits, sync bytes 2 and 3 are used.
                   1101 - 0x2D - ?
                   1102 - 0xD4 - ?
                   1103 - 0x00 -
                   1104 - 0x00 -
...
*/
#define RF_SYNC_CONFIG_5 0x11, 0x11, 0x05, 0x00, 0x01, 0x2D, 0xD4, 0x00, 0x00

/*
// PACKET FIELD PROPERTIES
...
// Configurations: 1200 - 0x81 - Use all 1s for the CRC Seed.
                                 ITU-T CRC8: X8+X2+X+1             
*/
#define RF_PKT_CRC_CONFIG_7 0x11, 0x12, 0x01, 0x00, 0x81//, 0x01, 0x08, 0xFF, 0xFF, 0x00, 0x02
...
// Descriptions:
//   PKT_LEN - This property configures the chip for reception of a variable length packet. This property is applicable only in RX mode,
//   and only when the automatic Packet Handler is enabled (i.e., PKT_CONFIG1:PH_RX_DISABLE is cleared). If enabled, the extracted 
//   length can be retrieved using the PACKET_INFO command.
//   PKT_LEN_FIELD_SOURCE - This property configures the chip for reception of a variable length packet, and defines where the length 
//   field is in the packet. The packet length byte(s) must be the last byte(s) in a fixed length field and must precede the variable
//   length field. This property is applicable only in RX mode, and only when the automatic Packet Handler is enabled 
//   (i.e., the PH_RX_DISABLE bit in the PKT_CONFIG1 property is cleared).
//   PKT_LEN_ADJUST - It is possible to define the total length of a packet in a variety of ways (e.g., including/excluding the
//   packet length bytes, including/excluding the CRC bytes, etc.) This property provides a means of adjusting/offsetting the
//   received packet length value in order to comply with a wide range of packet length implementations. This property is 
//   applicable only in RX mode, and only when the automatic Packet Handler is enabled (i.e., the PH_RX_DISABLE bit in the
//   PKT_CONFIG1 property is cleared). The value of the LEN_ADJUST field is added to the received value of the packet length byte(s),
//   extracted from the field specified by the SRC_FIELD value in the PKT_LEN_FIELD_SOURCE property. The resulting value is then 
//   used to configure the length of the variable length field specified by PKT_LEN:DST_FIELD. The required constant value of the 
//   LEN_ADJUST field is typically defined in a protocol specification or regulatory standard, or can be derived from the specification. The default processing mode of the chip is that the received packet length value does NOT include the packet length bytes, and does NOT include CRC bytes. In such a case, the LEN_ADJUST field should be set to 0x00. If the received packet length value includes the packet length bytes and/or the CRC bytes, its effective value must be reduced by adding a negative offset. In such a case, the LEN_ADJUST field should be set to the negative value of the extra byte count (in 2's complement format). Example: the transmitted packet has a 2-byte packet length field followed by a 10-byte Payload field, and the value contained in the packet length bytes = 0x000C = 12 bytes. The value of LEN_ADJUST should be set = 0xFE = -2. The LEN_ADJUST field is a signed value.
// Configurations: 1208 - 0x00 - The length field is least significant byte first.
//                               The length field is one byte in length.
//                               The data bytes containing the length field are not put in the fifo.
//                 1209 - 0X00 - 
//                 120A - 0x00 -
//                               
*/
#define RF_PKT_LEN_12 0x11, 0x12, 0x0C, 0x08, 0x00, 0x00, 0x00, 0x30, 0x30, 0x00, 0x0E, 0x04, 0x80, 0x00, 0x00, 0x00
...
// Descriptions:
//    PKT_FIELD_1_LENGTH_12_8 - The PKT_FIELD_1_LENGTH_XX properties specify the length of Field 1 in bytes. The field length is an 
//    unsigned 13-bit value; this property is Byte 1 of the length value, and is used along with the PKT_FIELD_1_LENGTH_7_0 property. 
//    Partitioning of the Payload into separate fields is provided to support variable length packets, and to allow different forms 
//    of data processing (e.g., Manchester encoding, data whitening, etc) across different fields. These properties are applicable 
//    in TX mode only if the START_TX command is sent with the parameter TX_LEN=0. These properties are applicable in RX mode only 
//    under the following conditions: RX Packet Handling is enabled by clearing PKT_CONFIG1:PH_RX_DISABLE, and The START_RX command
//    is sent with the parameter RX_LEN=0, and PKT_CONFIG1:PH_FIELD_SPLIT is cleared. If the PH_FIELD_SPLIT bit is set, the lengths
//    of the various fields in RX mode are determined by the PKT_RX_FIELD_LENGTH_XX properties. A value of zero in this property means 
//    that the field is not used. During TX mode, data will be retrieved from the TX FIFO until encountering the first field whose
//    length has been set to zero. During RX mode, data will be stored in the RX FIFO until again encountering the first field whose 
//    length has been set to zero. It is possible (although not common) to set the length of Field 1 = 0 bytes, as this would indicate 
//    transmission/reception of a packet with no Payload data at all. Field 1 cannot be configured as a variable length field, as there
//    is no preceding field that may contain the variable length byte(s).
//    PKT_FIELD_1_LENGTH_7_0 - Byte 0 of the unsigned 13-bit Field 1 length value. Please refer to the text description for 
//    PKT_FIELD_1_LENGTH_12_8 for more details regarding Field 1 length configuration.
//    PKT_FIELD_1_CONFIG - Partitioning of the Payload into separate fields is provided to support features such as variable length 
//    packets and field-specific data processing (e.g., Manchester encoding, data whitening, etc). This property provides configuration 
//    bits for field-specific processing on Field 1. The configuration bits in this property are common to both TX and RX modes 
//    dependent on PKT_CONFIG1:PH_FIELD_SPLIT; however, this property is only applicable when automatic Packet Handling is enabled 
//    (e.g., when using the TX FIFO as the modulation source in TX mode, or when PKT_CONFIG1:PH_RX_DISABLE is cleared in RX mode).
//    PKT_FIELD_1_CRC_CONFIG - This property is use to control the calculation, transmission, and checking of CRC across Field 1.
//    PKT_FIELD_2_LENGTH_12_8 - The PKT_FIELD_2_LENGTH_XX properties specify the length of Field 2 in bytes. The field length is an 
//    unsigned 13-bit value; this property is Byte 1 of the length value, and is used along with the PKT_FIELD_2_LENGTH_7_0 property. 
//    Field 2 may be configured as a variable length field by setting the appropriate value of DST_FIELD (in the PKT_LEN property); 
//    however, it is also necessary that a previous field contain the variable length byte(s). If configured as a variable length field, 
//    this property must be set with a value that represents the maximum expected length of the field. Please refer to the text 
//    description for PKT_FIELD_1_LENGTH_12_8 for more details regarding field length configuration.
//    PKT_FIELD_2_LENGTH_7_0 - Byte 0 of the unsigned 13-bit Field 2 length value. Please refer to the text description for 
//    PKT_FIELD_2_LENGTH_12_8 for more details regarding Field 2 length configuration.
//    PKT_FIELD_2_CONFIG - This property provides configuration bits for field-specific processing on Field 2. Please refer to the 
//    text description for PKT_FIELD_1_CONFIG for more details regarding enabling/disabling of field-specific processing.
//    PKT_FIELD_2_CRC_CONFIG - This property is use to control the calculation, transmission, and checking of CRC across Field 2. Please 
//    refer to the text description for PKT_FIELD_1_CRC_CONFIG for more details regarding configuration of field-specific CRC calculation.
//    PKT_FIELD_3_LENGTH_12_8 - The PKT_FIELD_3_LENGTH_XX properties specify the length of Field 3 in bytes. The field length is an 
//    unsigned 13-bit value; this property is Byte 1 of the length value, and is used along with the PKT_FIELD_3_LENGTH_7_0 property. 
//    Field 3 may be configured as a variable length field by setting the appropriate value of DST_FIELD (in the PKT_LEN property); 
//    however, it is also necessary that a previous field contain the variable length byte(s). If configured as a variable length field,
//    this property must be set with a value that represents the maximum expected length of the field. Please refer to the text 
//    description for PKT_FIELD_1_LENGTH_12_8 for more details regarding field length configuration.
//    PKT_FIELD_3_LENGTH_7_0 - Byte 0 of the unsigned 13-bit Field 3 length value. Please refer to the text description for 
//    PKT_FIELD_3_LENGTH_12_8 for more details regarding Field 3 length configuration.
//    PKT_FIELD_3_CONFIG - This property provides configuration bits for field-specific processing on Field 3. Please refer to the 
//    text description for PKT_FIELD_1_CONFIG for more details regarding enabling/disabling of field-specific processing.
//    PKT_FIELD_3_CRC_CONFIG - This property is use to control the calculation, transmission, and checking of CRC across Field 3. 
//    Please refer to the text description for PKT_FIELD_1_CRC_CONFIG for more details regarding configuration of field-specific
//    CRC calculation.
// Configurations: 120D - 0x00 -
*/
...
*/
#define RF_MODEM_FREQ_DEV_0_1 0x11, 0x20, 0x01, 0x0C, 0xD2 //0x5E
...
*/
#define RF_MODEM_BCR_OSR_1_9 0x11, 0x20, 0x09, 0x22, 0x03, 0x0D, 0x00, 0xA7, 0xC6, 0x00, 0x54, 0x02, 0xC2
...
*/
#define RF_MODEM_AFC_GEAR_7 0x11, 0x20, 0x07, 0x2C, 0x04, 0x36, 0xC0, 0x03, 0x30, 0xAF, 0xC0
...
*/
#define RF_MODEM_AGC_WINDOW_SIZE_9 0x11, 0x20, 0x09, 0x38, 0x11, 0xAB, 0xAB, 0x00, 0x1A, 0x14, 0x00, 0x00, 0x2B
...
*/
#define RF_MODEM_OOK_CNT1_11 0x11, 0x20, 0x0B, 0x42, 0xA4, 0x02, 0xD6, 0x83, 0x00, 0xAD, 0x01, 0x80, 0xFF, 0x0C, 0x00
...
*/
#define RF_MODEM_CHFLT_RX1_CHFLT_COE1_7_0_12 0x11, 0x21, 0x0C, 0x0C, 0x03, 0x00, 0x15, 0xFF, 0x00, 0x00, 0xC6, 0xC1, 0xB2, 0x9C, 0x80, 0x63
...
*/
#define RF_MODEM_CHFLT_RX2_CHFLT_COE7_7_0_12 0x11, 0x21, 0x0C, 0x18, 0x47, 0x2F, 0x1B, 0x0E, 0x05, 0x00, 0xFF, 0xFE, 0x00, 0x00, 0x00, 0x0F
...
*/
#define RF_FREQ_CONTROL_INTE_8 0x11, 0x40, 0x08, 0x00, 0x42, 0x08, 0xED, 0x91, 0xCC, 0xCD, 0x20, 0xFA  //167,79 MHz
